...
import { generateDemoTrace } from './demo-trace';
...
function resolveTrace(): { steps: TraceStep[]; source: string } {
  ...
  if (!arg) {
    const demoPath = path.join(process.cwd(), 'demo-trace.json');
    const steps = generateDemoTrace();
    fs.writeFileSync(demoPath, JSON.stringify(steps, null, 2));
    console.log(`\n  No file provided — generated demo trace at ${demoPath}\n`);
    return { steps, source: demoPath };
  }
